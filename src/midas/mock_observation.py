...
class WEAVE_Instrument(Instrument):
    ...
    def __init__(self, **kwargs):
        ...
        self.wavelength = arange(self.wave_init, self.wave_end, self.delta_wave) * u.angstrom
        # Wavelenght array for blue and red arms (used for saving each cube separately)
        self.wavelength_blue = arange(self.wave_init, self.wave_init + self.delta_wave * self.blue_arm_npix,
                                      self.delta_wave) * u.angstrom
        self.wave_blue_pos = where(self.wavelength <= self.wavelength_blue[-1])[0]
        self.wavelength_red = arange(self.wave_end - self.delta_wave * self.red_arm_npix, self.wave_end,
                                      self.delta_wave) * u.angstrom
        ...
        print('\n  · FoV: ({:.1f}, {:.1f})'.format(self.field_of_view[0], self.field_of_view[1])
              ...
              + '\n  · Wavelength range (angstrom): {:.2f} - {:.2f}'.format(self.wave_init, self.wave_end)
              + '\n    -> Blue arm: {:.2f} - {:.2f} ({} pixels)'.format(
            self.wavelength_blue[0], self.wavelength_blue[-1], self.wavelength_blue.size)
              + '\n    -> Red arm: {:.2f} - {:.2f} ({} pixels)'.format(
            self.wavelength_red[0], self.wavelength_red[-1], self.wavelength_red.size)
              )
    ...
    def compute_los_emission(self, stellar=True, nebular=False,
        ...
        if stellar:
            ...
            while part_i < n_stellar_part:
                print("\r Completion --> {}, {:2.2f} %".format(
