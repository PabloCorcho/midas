...
class WEAVE_Instrument(IFU):
    ...
    def detector_bins(self):
        ...
        self.X_kpc, self.Y_kpc = np.meshgrid(self.det_x_bins_kpc,
                                             self.det_y_bins_kpc)
...
class HI_Instrument(IFU):
    """This class represents a generic instrument that produces HI datacubes."""
    configfile = os.path.join(
        os.path.dirname(os.path.realpath(__file__)),
    ...
    def __init__(self, **kwargs):
        print('-' * 50
              + '\n [INSTRUMENT] Initialising instrument:\n'
              + '-' * 50)
        print('\n [INSTRUMENT] Loading default parameters for\n -- configfile: '
              + self.configfile)
        with open(self.configfile, 'r') as file:
            config = yaml.load(file, Loader=yaml.FullLoader)
            self.field_of_view = tuple(config['fov']) * u.arcsec  # arcsec
